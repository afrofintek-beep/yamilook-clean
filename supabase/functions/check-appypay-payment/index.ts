...
// Reconciles pending AppyPay purchases by querying the charge status (GET
// /v2.0/charges/{id}) and crediting the ones that are paid. Needed because the
// AppyPay webhook isn't reliably delivered — this is the source of truth.
// Callable by a signed-in user (checks their own pending purchases) or, when
// invoked with the service role + { all: true }, reconciles everything pending.
...
async function getAppyToken(): Promise<string | null> {
  ...
  const body = new URLSearchParams({
    grant_type: "client_credentials", client_id: key, client_secret: secret, resource: RESOURCE,
  });
...
serve(async (req) => {
  ...
  try {
    ...
    // Scope the pending set: a user's own purchases, or everything (service role).
    let query = admin.from("credit_purchases").select("id, provider_ref, merchant_ref, status").eq("status", "pending");
    if (all === true) {
    ...
    for (const p of pending) {
      ...
      if (status === "success") {
        await admin.rpc("fulfill_credit_purchase", { p_purchase_id: p.id, p_provider_ref: p.provider_ref ?? null });
        credited++;
      }
    }
    return json({ checked: pending.length, credited });
  } catch (e) {
