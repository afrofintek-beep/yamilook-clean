...
// Reconciles pending Kumbu purchases by querying the AppyPay charge status and
// crediting the paid ones (the webhook isn't reliably delivered). Mirrors
...
serve(async (req) => {
  ...
  try {
    ...
    let query = admin.from("kumbu_purchases").select("id, provider_ref, merchant_ref, status").eq("status", "pending");
    if (all === true) {
    ...
    for (const p of pending) {
      ...
      if (status === "success") {
        await admin.rpc("fulfill_kumbu_purchase", { p_purchase_id: p.id, p_provider_ref: p.provider_ref ?? null });
        credited++;
      }
    }
