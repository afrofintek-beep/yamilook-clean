...
import { useCalls, CallHistory, ScheduledCall } from '@/hooks/useCalls';
...
import { OnlineStatus, formatLastSeen } from '@/components/ui/OnlineStatus';
...
export default function Calls() {
  ...
  const handleStartCall = useCallback(async (userId: string, type: 'voice' | 'video') => {
    if (activeCall) {
      await activeCall.startCall(userId, type);
    } else {
  ...
  const getCallIcon = (call: CallHistory, userId: string) => {
    if (call.status === 'missed' || call.status === 'declined') {
      return <PhoneMissed className="w-4 h-4 text-destructive" />;
    }
    // Check if user was the initiator - would need initiator_id in CallHistory
    // For now, we'll just show outgoing/incoming based on some logic
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <header className="sticky top-0 z-50 bg-background border-b border-border/50 safe-top">
        <div className="flex items-center justify-between px-4 py-3">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-xl font-bold truncate">{t('calls.title')}</h1>
          </div>
          <DropdownMenu>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="flex-1 flex flex-col">
        <TabsList className="mx-4 mt-4 bg-secondary/80 p-1">
          <TabsTrigger 
            value="all" 
      ...
      {/* FAB */}
      <button 
        className="fixed bottom-6 right-6 w-14 h-14 rounded-2xl bg-gradient-primary text-white shadow-xl flex items-center justify-center hover:scale-105 transition-all hover:shadow-2xl active:scale-95"
        onClick={() => setScheduleOpen(true)}
        style={{ boxShadow: '0 8px 30px -4px hsl(var(--primary) / 0.5)' }}
      >
